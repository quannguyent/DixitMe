...
import (
	...
	models "dixitme/internal/data/models" // Alias to models to keep code changes minimal
...
func GetBotStats(c *gin.Context) {
	...
	db.Model(&models.GamePlayer{}).
		...
		Where("players.type = ?", domain.PlayerTypeBot). // Use domain.PlayerTypeBot
	...
	`, domain.PlayerTypeBot, "easy").Scan(&stats.EasyBotWins)
	...
	db.Raw(`
		...
		JOIN players p ON gh.winner_id = p.id 
		WHERE p.type = ? AND p.bot_level = ?
	`, domain.PlayerTypeBot, "medium").Scan(&stats.MediumBotWins)
	...
	db.Raw(`
		...
		JOIN players p ON gh.winner_id = p.id 
		WHERE p.type = ? AND p.bot_level = ?
	`, domain.PlayerTypeBot, "hard").Scan(&stats.HardBotWins)
	...
	db.Raw(`
		...
		WHERE p.type = ?
	`, domain.PlayerTypeBot).Scan(&stats.AvgBotScore)
