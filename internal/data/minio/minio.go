...
// New creates a MinIO client without mutating global state.
func New(cfg config.MinIOConfig) (*MinIOClient, error) {
	...
	if err := client.SetBucketPolicy(ctx, cfg.BucketName, policy); err != nil {
		logger.GetLogger().Warn("Failed to set bucket policy", "error", err)
	}

	return wrapped, nil
}

// Initialize sets up MinIO client and stores it for legacy callers.
func Initialize(cfg config.MinIOConfig) (*MinIOClient, error) {
