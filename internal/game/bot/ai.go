...
// DataProvider defines the interface for data access required by the bot
type DataProvider interface {
	GetCardTags(cardID int) ([]domain.Tag, error)
	GetCard(cardID int) (*domain.Card, error)
}
...
func Initialize(provider DataProvider) {
...
func GetBotManager() *BotManager {
	if botManager == nil {
		// Should not happen if Initialize is called first
...
func (bp *BotPlayer) creativeClue(tags []domain.Tag) string {
...
func (bp *BotPlayer) getCardTags(cardID int) []domain.Tag {
	if bp.dataProvider == nil {
		return []domain.Tag{}
	}

	tags, err := bp.dataProvider.GetCardTags(cardID)
	if err != nil {
...
func (bp *BotPlayer) getCardDetails(cardID int) *domain.Card {
	if bp.dataProvider == nil {
		return nil
	}

	card, err := bp.dataProvider.GetCard(cardID)
	if err != nil {
