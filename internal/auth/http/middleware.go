...
func AuthMiddleware(jwtService *auth.JWTService, required bool) gin.HandlerFunc {
...
func RequireAdmin(jwtService *auth.JWTService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !exists || userInfo.AuthType == domain.AuthTypeGuest { // Use domain.AuthTypeGuest
...
func GuestOrAuth(jwtService *auth.JWTService) gin.HandlerFunc {
	return func(c *gin.Context) {
		token := extractToken(c)
...
func IsGuest(c *gin.Context) bool {
	...
	return userInfo.AuthType == domain.AuthTypeGuest
}
